...
Bluetooth キーボード／マウスとして入力先へ接続する拡張案と、他社リモートデスクトップ／visionOS アプリへ入力する際の条件は [入力先を増やすための設計](docs/HID_ARCHITECTURE.md) にまとめています。現在の公開版に BLE HID 機器用ファームウェアは含まれません。
